...
namespace study_cases {
	...
	void sim_106_timed_refresh(int v) {
		sim_106_render();
		...
		glutTimerFunc(1000/FPS, sim_106_timed_refresh, v);
	}

	void sim_106_init_agent(agent_particle& a) {
	...
	void sim_106_init_simulation() {
		...
		a.target = vec3(0.75f, 0.0f, 50.0f);
		a.cur_pos = vec3(0.75f, 0.0f, 0.0f);
		a.cur_vel = vec3(0.5f,0,1);
		sim_106_init_agent(a);

		idx = S.add_agent_particle();
		agent_particle& b = S.get_agent_particle(idx);
		b.target = vec3(-0.75f, 0.0f, 50.0f);
		b.cur_pos = vec3(-0.75f, 0.0f, 0.0f);
		b.cur_vel = vec3(-0.5f,0,1);
		sim_106_init_agent(b);

		S.set_particle_particle_collisions(true);
		S.set_time_step(0.001f);
	}

	void sim_106_init_geometry() {
	...
	int sim_106_parse_arguments(int argc, char *argv[]) {
	...
	int sim_106_init(bool init_window) {
		width = 640;
		height = 480;
	...
	void sim_106_regular_keys_keyboard(unsigned char c, int x, int y) {
		...
		switch (c) {
		case 'h': sim_106_usage(); break;
		case 'r': sim_106_exit(); sim_106_init(false); break;
		}
	}
	...
	void sim_106(int argc, char *argv[]) {
		...
		atexit(sim_106_exit);
		glutDisplayFunc(sim_106_render);
		...
		glutTimerFunc(1030.0f/charanim::FPS, sim_106_timed_refresh, 0);
